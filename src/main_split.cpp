...
int main() {
	...
	std::string model_name = "example_mesh.stl";
    std::string output_name = "example_mesh_split";
	...
	config.min_center_distance_rel = 4.0f;
	...
	export_to_vtk(sp, output_name + "_original.vtk");
	std::cout << "Original SpherePack exported to: " << output_name + "_original.vtk" << std::endl;
    ...
    config.min_radius_vox = 10; // Adjust min radius for splitting if needed
	...
	// save_voxel_grid_to_vtk(labeled_grid, output_name + "_labeled_grid.vtk"); // Uncomment if function exists

	// 4. Export all fragments to VTK
	for (size_t i = 0; i < fragments.size(); ++i) {
		std::string frag_name = output_name + "_fragment_" + std::to_string(i+1) + ".vtk";
		export_to_vtk(fragments[i], frag_name);
		std::cout << "Fragment " << (i+1) << " exported to: " << frag_name << std::endl;
		std::cout << "  Spheres: " << fragments[i].num_spheres() << ", Volume: " << fragments[i].mass << std::endl;
	}
